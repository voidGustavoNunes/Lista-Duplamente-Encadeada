...
void inicializa(TLista *L){
	...
	for(i=0; i<10; i++){
			...
			printf("\nAgora digite sua posicao: ");
			scanf("%d", &L->produto[i].posicao);
...
void inserirVenda(TLista *L){
	...
	TVenda *produto = (TVenda *)malloc(sizeof(TVenda));
	...
	for(i=0; i<10; i++){ //10 produtos 
		if(strcmp(nome, L->produto[i].nome) == 0 && produto == L->produto[i].posicao){
			...
			if(L->vendaInicial == NULL){
				novo->vendaInicial->inicio = produto;
				novo->vendaInicial->fim = produto;
			}
			else{
				while(atual != NULL){
					inserido = 1;
					...
					if(anterior == NULL){
						novo->vendaInicial->ante = produto;
					}
					else{
